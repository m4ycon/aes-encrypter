...
def sub_bytes(state: list[list[int]], crypt: bool = True):
  for r in range(0, 4):
    for c in range(0, 4):
      state[r][c] = SBOX[state[r][c]] if crypt else INV_SBOX[state[r][c]]
  return state

def shift_rows(state: list[list[int]], crypt: bool = True):
  for rows in range(0, 4):
    state[rows] = state[rows][rows:] + state[rows][:rows] if crypt else state[rows][-rows:] + state[rows][:-rows]
  return state
...
def cipher(plain_text: str, w: str):
  ...
  for i in range(0, len(plain_text), 16):
    res += cipher16(plain_text[i:i+16], w)
  return res

def cipher16(plain_text: str, w: str):
